...
function addRemoteGit (spec, _cb) {
  ...
  if (toGet.hosted) {
    ...
    if (toGet.getDefaultRepresentation() !== 'shortcut') {
...
function tryGitProto (toGet, cb) {
  ...
  if (!gitURL) return trySSH(toGet, cb)
...
function trySSH (toGet, cb) {
  ...
  if (!sshURL) return tryHTTPS(toGet, cb)

  log.silly('trySSH', 'attempting to clone', sshURL)
  tryClone(toGet, sshURL, false, function (er) {
...
function tryHTTPS (toGet, cb) {
  ...
  tryClone(toGet, httpsURL, true, cb)
}
...
function getResolved (uri, treeish) {
  return npa.resolve(null, uri + '#' + treeish).saveSpec
}
