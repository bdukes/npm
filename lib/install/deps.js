...
function doesChildVersionMatch (child, requested, requestor) {
  ...
  if (!childReq) childReq = npa.resolve(moduleName(child), child.package._from.replace(new RegExp('^' + moduleName(child) + '@'), ''))
  if (childReq) {
    if (childReq.rawSpec === requested.rawSpec) return true
    if (childReq.type === requested.type && childReq.fetchSpec === requested.fetchSpec) return true
    if (childReq.type === requested.type && childReq.spec === requested.fetchSpec) return true
  ...
  if (!registryTypes[requested.type]) {
    if (child.package._from) {
      ...
      if (fromReq.type === requested.type && fromReq.fetchSpec === requested.fetchSpec) return true
    }
    return false
  }
...
function childDependencySpecifier (tree, name, spec, cb) {
  try {
    cb(null, npa.resolve(name, spec, packageRelativePath(tree)))
  } catch (err) {
    return cb(err)
  }
}
